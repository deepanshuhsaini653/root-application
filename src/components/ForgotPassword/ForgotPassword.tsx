...
const schema = yup.object({
  ...
  phone: yup.string().when("isEmail", {
    ...
    then: yup
      .string()
      .required("Required")
      .matches(REGREX.phone, "Enter valid phone number"),
  }),
});

const ForgotPasswordComponent: FC<IForgotPasswordProps> = (
  props
): JSX.Element => {
  const {
    onSubmitData,
    ...
    <Box>
      <Box sx={{ marginBottom: "3rem", textAlign: "center" }}>
        ...
        <Typography sx={{ color: "#7B7887", fontSize: ".9rem" }}>
          {type === "mobile"
            ? "Enter your registeration mobile no. on which OTP will be send"
            : "Enter your registeration email id on which mail will be send"}
        </Typography>
      </Box>
      <Formik
        validationSchema={schema}
        initialValues={{ phone: "", email: "", code: "" }}
              ...
              <Grid item xs={12}>
                ...
                >
                  <Typography sx={{ fontSize: "1rem" }}>
                    {type === "mobile" ? "Request OTP" : "Forgot password"}
                  </Typography>
                </LoadingButton>
              </Grid>
              <Grid item xs={12}>
