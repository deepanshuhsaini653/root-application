...
import { Grid, TextField, BaseTextFieldProps, Button, Typography, IconButton, InputAdornment, Box } from '@mui/material';
...
export const ResetPassword: FC<IResetPasswordProps> = (props) => {
    ...
    const viewConfPasswordHandler = () => setVisibleConfPassword(!visibleConfPassword);
    ...
    return (
        <Box>
            <Box sx={{ marginBottom: '3rem', textAlign: 'center' }}>
                ...
                <Typography sx={{ color: '#7B7887', fontSize: '.9rem' }}>Your new password must be diffrent from previous used password.</Typography>
            </Box>
            <Formik validationSchema={schema} initialValues={{ password: '', confirmPassword: '' }} onSubmit={onSubmitHandler}>
                            ...
                            <Grid item xs={12} lg={12} pb={6}>
                                <TextField
                                    ...
                                    InputProps={{
                                        endAdornment: (
                                            <InputAdornment position="end">
                                                <IconButton aria-label="toggle password visibility" onClick={viewConfPasswordHandler} edge="end">
                                                    {visibleConfPassword ? <VisibilityOff /> : <Visibility />}
                                                </IconButton>
                                            </InputAdornment>
                                        )
                                    }}
